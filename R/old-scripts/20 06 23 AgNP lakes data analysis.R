  ...
  z = lm(Log.Tag.excretion ~ Year, data = NPexcr)$residual
  n = length(z)
  ...
  aov.Nx <- aov(Log.N.excretion ~ Lake*Year, 
               data = NPexcr)
  # not looking at year effect because very low sample sizes for L239 
  # (1 in 2014 vs. 7 in 2015) --> relevant only if get more samples in 2014 
  ...
  windows(width = 14, height = 7)
  ggplot(NPexcr.ts, aes(x = Year, y = log(N.excretion.av), color = Lake,
                        group = Lake)) +
    geom_point(size = 5) +
    geom_line(size = 2) +
    theme_classic(base_size = 26) +
  ...
  aov.Px <- aov(Log.P.excretion ~ Lake*Year, 
               data = NPexcr) # log-transformed because residuals not normally distributed
  ...
  # ..excreted N:P (molar) ----
  windows(width = 14, height = 7)
  ggplot(NPexcr, aes(x = Lake, y = Excreted.NP, fill = Lake)) +
    theme_classic(base_size = 26) +
    geom_boxplot(alpha = 0.7, outlier.size = 3, outlier.colour = NULL) +
    #geom_jitter() +
    theme_bw(base_size = 20) +
  ...
  # ...time series 2012 to 2015 ----
  windows(width = 14, height = 7)
  ggplot(NPexcr.ts, aes(x = Year, y = Excreted.NP.av, color = Lake,
                        group = Lake)) +
    geom_point(size = 5) +
    geom_line(size = 2) +
    theme_classic(base_size = 26) +
  ...
  # ..excreted C:N (molar) ----
  windows(width = 14, height = 7)
  ggplot(NPexcr, aes(x = Lake, y = Excreted.CN, fill = Lake)) +
    theme_classic(base_size = 26) +
    geom_boxplot(alpha = 0.7, outlier.size = 3, outlier.colour = NULL) +
    #geom_jitter() +
    theme_bw(base_size = 20) +

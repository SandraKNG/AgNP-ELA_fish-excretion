  ...
  histkdnc(NPexcr$Log.massnorm.N.excr) # normal distribution
  ...
  skewness(NPexcr$N.excretion, na.rm = T) 
   plot(NPexcr$Year, resid(lm(massnorm.N.excr ~ Lake*Year, 
  ...
  # Tag or N excretion
  z = glm(Log.massnorm.N.excr ~ Lake*Year, 
         data = NPexcr %>%  filter(Year != '2014'))$residual
  n = length(z)
  ...
  glm.Nx <- glm(Log.N.excretion ~ Lake*Year*Mass, 
                data = NPexcr %>% filter(Year != '2014'))
  glm.Nxcorr <- glm(Log.massnorm.N.excretion ~ Lake*Year, 
                data = NPexcr %>% filter(Year != '2014'), family = 'gamma')
  aov.Nxcorr <- aov(Log.massnorm.N.excr ~ Lake*Year, 
                     data = NPexcr %>% filter(Year != '2014'))
  ...
  lm.Nxcorr <- gls(massnorm.N.excr ~ Lake*Year, 
                    data = NPexcr %>% filter(Year != '2014'), na.action = na.omit)
  vFixed <- varFixed (~Mass)
  gls.Nxcorr <- gls(massnorm.N.excr ~ Lake*Year,
                    weights = vFixed,
                    data = NPexcr %>% filter(Year != '2014'), na.action = na.omit)
  anova(lm.Nxcorr, gls.Nxcorr)
  ...
  # ...time series 2012 to 2015 ----
  windows(width = 14, height = 7)
  ggplot(NPexcr.ts, aes(x = Year, y = P.excretion.av, color = Lake,
                        group = Lake)) +
    geom_point(size = 5) +
    geom_line(size = 2) +
    theme_classic(base_size = 26) +

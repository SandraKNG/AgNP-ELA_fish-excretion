  ...
  Pexcrmod.p <- ggplot(Pmod,aes(x = Ingestion.rate, y = ModP.excretion,
    ...
    annotate("text", x = 0.13, y = 0.0011, label = 'Food C:P min',
             size = 6, fontface = 'bold') +
    annotate("text", x = 0.13, y = 0.0006, label = 'Food C:P max',
             size = 6, fontface = 'bold') +
  ...
  Pexcrmeas.p <- ggplot(Pmod, aes(x = Ingestion.rate, y = MeasP.excretion,
                                 color = Lake)) +
    geom_point(data = Pmod %>%  filter(Year == '2012'),
               aes(x = 0, shape = Year, color = Lake),
                               size = 7.5) +
    geom_point(data = Pmod %>%  filter(Year == '2014'),
               aes(x = 0.1, shape = Year, color = Lake),
               size = 7.5) +
    geom_point(data = Pmod %>%  filter(Year == '2015'),
               aes(x = 0.2, shape = Year, color = Lake),
               size = 7.5) +

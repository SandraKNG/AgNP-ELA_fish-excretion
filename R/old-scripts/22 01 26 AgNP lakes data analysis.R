  ...
  plot_N <- 
    ...
    geom_point(aes(x = Mass..g., y = N.excretion, color = Lake, shape = Year), 
               data = NPexcr %>%  filter(Year != '2014'),
               size = 3) +
    labs(x = "Mass (g)", y = "N excretion rate (μg/ind/h)") +
    scale_colour_manual(name = 'Lake',
                        labels = c('AgNPs 222', 'Reference 239'),
                        values = c("black","gray60")) +
  ...
  plot_P <- 
    ...
    geom_point(aes(x = Mass..g., y = P.excretion, color = Lake, shape = Year), 
               data = NPexcr, size = 3) +
    labs(x = "Mass (g)", y = "P excretion rate (μg/ind/h)") +
    scale_colour_manual(name = 'Lake',
                        labels = c('AgNPs 222', 'Reference 239'),
                        values = c("black","gray60")) +
    scale_shape_manual(labels = c('2012', '2014', '2015'),
  ...
  ggsave('final figures/Fig 4.2.png', 
         width = 14, height = 7, 
         units = 'in', dpi = 300)
  ...
  plot_Ic <- 
    ...
    scale_fill_brewer() +
    theme_classic(base_size = 18) + 
    labs(x = "Mass (g)", y = "C ingestion rate (μg/ind/h)") 
  plot_Ic
  
  # plot sensitivity to changes in diet on CNP limitation
  FStoichm.sens <- sensitivity(TL = 6, param = list(k_sd = 0.2, Dn_sd = 0.2, Dc_sd = 0.1, Dp_sd = 0.2),
              par = c("k_sd","Dn_sd","Dc_sd","Dp_sd"), out = c("Fn", "Fp", "Ic"))
  limitation(FStoichm, plot = TRUE)
  
  # plot N/P/C limitation, N/P excretion, Ingestion rate, Growth rate
  windows(width = 14, height = 7)
  limitation(FStoichm) + theme_classic(base_size = 26)
  ...
  Pexcrmod.p <- fishflux::plot_cnp(FStoichm,  y = "Fn", x = "Ic", probs = c(0.5, 0.8, 0.95)) +
    geom_point(data = NPexcr, aes(x = Total.length.cm, 
                                  y = N.excretion.d, 
                                  color = Lake, shape = Year), size = 3) +
    ylab("N excretion (g/d)") +
    theme_classic(base_size = 26) +
    theme(text = element_text( family = "Tahoma"),
          axis.title = element_text(face = "bold"),
          legend.title = element_text(face = 'bold'),

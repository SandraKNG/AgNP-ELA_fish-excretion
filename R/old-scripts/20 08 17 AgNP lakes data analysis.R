  ...
  library(emmeans) # for lsmeans command for pairwise comparisons post glm
  ...
  NPexcr %>% summarize(min(NPexcr$massnorm.N.excr, na.rm = TRUE)) 
  # smallest value = 16.78418
  NPexcr %>% summarize(min(NPexcr$LevDevN, na.rm = TRUE))
  quantile(NPexcr$LevDevN, probs = c(0, .01, .05, 0.025, .5), na.rm = TRUE)
  # smallest value = 0.002616777
  NPexcr$LevDevN2 <- NPexcr$LevDevN + 1.7
  ...
  estimate_modelN <- emmeans(glm.LevDevN_gamma, ~ Lake|Year, type = 'response')
  ...
  glm.Nxnorm_emmeans <- emmeans(glm.Nxnorm, ~ Lake|Year, type = 'response')
  plot(glm.Nxnorm_emmeans)
  ...
  NPexcr$LevDevP <- with(NPexcr, 
                         LeveneDeviates(massnorm.P.excr,
                                        ...
                                        med  = TRUE, log_trans = TRUE))
  # add 10% of the smallest value
  NPexcr %>% summarize(min(NPexcr$massnorm.P.excr, na.rm = TRUE)) 
  # smallest value = 1.891182
  NPexcr %>% summarize(min(NPexcr$LevDevP, na.rm = TRUE)) 
  # smallest value = 0.0008525152
  NPexcr$LevDevP2 <- NPexcr$LevDevP + 0.2
  ...
  emmeans(glm.LevDevP_gamma, pairwise ~ Lake:Year)
  
  summary(aov.Pnx)
  summary(aov.Px)
  summary(glm.Px)
  summary(glm.Pxnorm, pairwise)

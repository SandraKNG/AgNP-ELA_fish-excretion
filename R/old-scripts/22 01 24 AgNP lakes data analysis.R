  ...
  # Yellow perch
  FStoichm <- cnp_model_mcmc(TL = 4:11, param = parammod, iter = 5000)
  ...
  iter <- (lapply(FStoichm$stanfit, FUN = function(x){
    rstan::extract(x, c("Fn", "Fp", "w1", "Ic", "IN"))})) %>%
    lapply(FUN = get_iter) %>%
    dplyr::bind_rows()
  iter$tl <- rep(NPexcr$Total.length..mm., each = 2500)
  iter$mass <- rep(NPexcr$Mass, each = 2500)

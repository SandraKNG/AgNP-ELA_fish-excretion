  ...
  NPexcr.ss1 <- NPexcr %>% group_by(Lake) %>%
    describe_distribution(list('Mass', 'massnorm.N.excretion', 'massnorm.P.excretion')) #%>% 
    # select(c('Mass', 'massnorm.N.excretion', 'massnorm.P.excretion'))
  ...
  ggplot(NPexcr, 
    ...
    # geom_jitter(size = 2, position = position_jitterdodge(jitter.width = 0.2),
    ...
    annotate("text", x = 1.82, y = 1.40, label = 'a', 
             size = 7, fontface = 'bold') +
    annotate("text", x = 2.19, y = 0.5, label = 'b', 
             size = 7, fontface = 'bold', color = 'gray60') +
  ...
  # emmeans
  lm.Pxnorm_emmeans <- emmeans(lm.Pxnorm, ~ Lake|Year, 
                               bias.adj = TRUE)
  ...
  ggarrange(Nexcr.p, eff_sizeN.p, Pexcr.p, eff_sizeP.p, NPexcr.p, 
            ...
            font.label = list(size = 26), label.x = 0.15, label.y = 1,
  ...
  t.Tag <- t.test(massnorm.Tag.excr ~ Year, NPexcr, var.equal = F)
  t.Tag
  
  var.test(massnorm.NAg.excr ~ Year, NPexcr, alternative = 'two.sided')
  t.NAg <- t.test(massnorm.NAg.excr ~ Year, NPexcr, var.equal = T)
  t.NAg
  
  var.test(massnorm.PAg.excr ~ Year, NPexcr, alternative = 'two.sided')
  t.PAg <- t.test(massnorm.PAg.excr ~ Year, NPexcr, var.equal = F)
  t.PAg
  ...
  TAgexcrp <- ggplot(NPexcr %>% filter(Year != 2012), 
    ...
    # geom_errorbar(aes(ymin = massnorm.Tag.excr.av - massnorm.Tag.excr.sd,
    #                   ymax = massnorm.Tag.excr.av + massnorm.Tag.excr.sd),
    #               width = 0.2, lwd = 1.5, position = position_dodge(0.5)) +
    ...
    theme(text = element_text( family = "Tahoma"),
          ...
          legend.key.width = unit(3, 'lines'),
          legend.position = 'right') +
    scale_y_continuous(name = 'Ag excretion (μg Ag/g/h)') +
    scale_x_discrete(labels = c('Year 1', 'Year 2')) +
    annotate("text", x = 1, y = 25, label = 'a', 
             size = 8, fontface = 'bold') +
    annotate("text", x = 2, y = 56, label = 'b', 
             size = 8, fontface = 'bold') 
  TAgexcrp
  ...
  NAgexcrp <- ggplot(NPexcr %>% filter(Year != 2012), 
    ...
    # geom_errorbar(aes(ymin = massnorm.NAg.excr.av - 300,
    #                   ymax = massnorm.NAg.excr.av + massnorm.NAg.excr.sd),
    #               width = 0.2, lwd = 1.5, position = position_dodge(0.5)) +
    # geom_bar(stat = "identity", width = 0.5, color="black") +
    # geom_point(size = 5, position = position_dodge(0.5)) +
    geom_boxplot(outlier.shape = NA, fill = 'gray') +
    ...
    theme(text = element_text( family = "Tahoma"),
          ...
          legend.key.width = unit(3, 'lines'),
          legend.position = 'right') +
    scale_y_continuous(name = 'N:Ag excretion (molar)') +
    scale_x_discrete(labels = c('Year 1', 'Year 2')) +
    annotate("text", x = 1, y = 1100, label = 'a',
             size = 8, fontface = 'bold') +
    annotate("text", x = 2, y = 2150, label = 'a',
             size = 8, fontface = 'bold')
  NAgexcrp
  ...
  PAgexcrp <- ggplot(NPexcr %>% filter(Year != 2012), 
    ...
    # geom_errorbar(aes(ymin = massnorm.PAg.excr.av - massnorm.PAg.excr.sd,
    #                   ymax = massnorm.PAg.excr.av + massnorm.PAg.excr.sd),
    #               width = 0.2, lwd = 1.5, position = position_dodge(0.5)) +
    # geom_bar(stat = "identity", width = 0.5, color="black") +
    # geom_point(size = 5, position = position_dodge(0.5)) +
    geom_boxplot(outlier.shape = NA, fill = 'gray') +
    ...
    theme(text = element_text( family = "Tahoma"),
          ...
          legend.key.width = unit(3, 'lines'),
          legend.position = 'right') +
    scale_y_continuous(name = 'P:Ag excretion (molar)') +
    scale_x_discrete(labels = c('Year 1', 'Year 2')) +
  annotate("text", x = 1, y = 36, label = 'a',
           size = 8, fontface = 'bold') +
  annotate("text", x = 2, y = 84, label = 'b',
           size = 8, fontface = 'bold')
  PAgexcrp
  
  ggarrange(TAgexcrp, NAgexcrp, PAgexcrp, ncol = 1, nrow = 3, 
            labels = c("(a)", "(b)", "(c)"), 
            font.label = list(size = 20), label.x = 0.09, label.y = 1,
  ...
  Pexcr_mod.p <- 
    ...
    geom_point(aes(x = Mass, y = P.excretion, color = Lake, shape = Year), 
               data = NPexcr, size = 4) +
    labs(x = "Dry mass (g)", y = "P excretion rate (μg P/ind/h)") +
    scale_colour_manual(name = 'Lake',
                        labels = c('AgNPs 222', 'Reference 239'),
                        values = c("black","gray60")) +
  ...
  fig4.legend <- get_legend(Pexcr_mod.p)
  
  windows(width = 14, height = 7)
  ggarrange(Nexcr_mod.p, Pexcr_mod.p, ncol = 2,
  ...
  plot_grid(lim_ref.p, Cing_ref.p, Nexcr_ref.p, Pexcr_ref.p, 
            ...
            label_size = 18)
  ggsave('final figures/Fig 3.1.png', 
         width = 12, height = 7, 
         units = 'in', dpi = 300)
  ...
  # ggarrange(lim_mod.p, Cing_mod.p, ncol = 2,
  #           legend = 'bottom', common.legend = F, align = 'h', labels = c('(a)', '(b)'),
  #           font.label = list(size = 26), label.x = 0.16, label.y = 1.01)
